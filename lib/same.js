...
class Same extends Format {
  ...
  nodeId () { return this.spaceIfNoKey(super.nodeId()) }
  ...
  mapBody () {
    ...
    for (const [key, val] of objEnt) {
      if (this.expect.has(key)) {
        const me = this.mapEntry(key, val, key)
        out += me
        seen.add(key)
        continue
      }
      ...
      for (const [expkey, expval] of this.expect.entries()) {
        ...
        if (s.match) {
          sawMatch = true
          seen.add(expkey)
          const me = this.mapEntry(key, val, expkey)
          out += me
          break
        }
      }
  ...
  diff () {
    ...
    const pobj = this.prefix(this.simplePrint(obj), '+').slice(0, -1)
    return `${pexp}\n${pobj}\n`

    return this.prefix(this.simplePrint(expect), '-') +
      this.prefix(this.simplePrint(obj), '+')
  }
