...
// Handle sigterm signals, since this usually indicates a timeout
process.on('SIGTERM', function onTerm () {
  process.removeListener('SIGTERM', onTerm)
  if (process.listeners('SIGTERM').length === 0) {
    // if this was the only SIGTERM handler, then it means we're
    // being forcibly killed.  Log the open handles, and then fail.
    ...
    setTimeout(function () {
      // kill this pid, make sure it has time to catch the signal.
