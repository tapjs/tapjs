...
Test.prototype._runChild = function runChild (child, name, extra, cb) {
  ...
  child.on('end', function () {
    ...
    if (child._threw) {
      extra.error = child._threw.message
    }
    self.ok(child._ok, name, extra)
    if (!self._ended && !self._syncEnd) {
      self.push('\n')
    }
  })
  ...
  // If the child tried to end while initializing, probably via plan(),
  // and didn't return a promise, then we end it now.
  if (child._syncEnd && !child._promise) {
    // console.error('process sync end', child._name)
    child._initialized = true
    try {
      child.end(child._syncEnd[0])
    } catch (er) {
      console.error('threw', er)
      child.threw(er)
    }
  }
...
Test.prototype.done = Test.prototype.end = function end (implicit) {
  ...
  if (this._initializing && !this._initialized) {
    // console.error('parent and !initialized?')
    this._syncEnd = [implicit]
    return
  }
