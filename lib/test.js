...
Test.prototype.test = function test (name, extra, cb, deferred) {
  ...
  var variableArgs = Array.prototype.slice.call(arguments, 0, 3);
  
  if (typeof variableArgs[0] === 'string'){
    name = variableArgs[0]
    variableArgs.shift()
  }
  else {
  ...
  if (typeof variableArgs[0] === 'object'){
    extra = variableArgs[0]
    variableArgs.shift()
  }
  else {
    extra = {}
  ...
  if (typeof variableArgs[0] === 'function'){
    cb = variableArgs[0]
    variableArgs.shift()
  }
  else {
  ...
  if (variableArgs.length > 0){
    throw new Error('invalid test argument(s)')
  }
