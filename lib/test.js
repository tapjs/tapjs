...
Test.prototype.test = function test (name, extra, cb) {
  ...
  if (extra._before === this || extra._after === this) {
    child._before = extra._before
    child._after = extra._after
    delete extra._before
    delete extra._after
    this._runChild(child, name, extra, cb)
  } else {
    this._runBeforeEach(child)
    this._runChild(child, name, extra, cb)
  }
}

Test.prototype._runBeforeEach = function (who) {
  if (this._parent) {
    this._parent._runBeforeEach(who)
  }
  loop(who, this._beforeEach, 'before')
}

Test.prototype._runAfterEach = function (who) {
  loop(who, this._afterEach, 'after')
  if (this._parent) {
    this._parent._runAfterEach(who)
  }
}

function loop (self, arr, which) {
  for (var i = 0; i < arr.length; i++) {
    var name = which + ': ' + self._name
    var extra = {}
    extra['_' + which] = self
    var cb = arr[i]
    self.test(name, extra, cb)
  }
  return
  ...
  function loop_ (i) {
    ...
    var name = which + ': ' + self.name
    var extra = {}
    extra[which] = self
    self.test(name, extra, cb)

    arr[i].call(self, function (er) {
      if (er) {
...
Test.prototype.done = Test.prototype.end = function end (implicit) {
  ...
  var self = this
  if (this._parent &&
      this._before !== this._parent &&
      this._after !== this._parent) {
    this._parent._runAfterEach(this)
  }
