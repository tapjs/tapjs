...
var bindObj = require('./bind-obj.js')
...
function Test (options) {
  ...
  bindObj(this, this)
  bindObj(this, Test.prototype)
}

Test.prototype.spawn = function spawnTest (cmd, args, options, name) {
...
Test.prototype.test = function test (name, extra, cb) {
  ...
  return this.sub(Test, extra)
}

Test.prototype.stdin = function (name, extra) {
  return this.sub(Stdin, extra)
}
...
Test.prototype.main = function (cb) {
  ...
  var d = domain.create()
  d.add(this)
  d.on('error', this.threw.bind(this))

  var self = this
  try {
    // Note: if the callback calls t.end(), we have to defer it until
    // AFTER this block, or else the parent will move onto the next
    // thing while promise is still pending.
    // var ret = this.cb(this)
    var ret = d.run(this.cb.bind(this, this))
    if (ret && ret.then) {
      ret.then(function () {
        self.end(IMPLICIT)
        done()
