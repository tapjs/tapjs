...
const bannerWords = '  🌈 FINAL RESULTS 🌈  '
const banner = (
  '\n' +
  chalk.bgHex('#fff')(s(bannerWords.length)) +
  '\n' +
  chalk.bgHex('#fff')(chalk.hex('#000')(chalk.bold(bannerWords))) +
  '\n' +
  chalk.bgHex('#fff')(s(bannerWords.length)) +
  '\n'
)
...
module.exports = ({ results, counts, lists }) => {
  try {
    return summary({results, counts, lists})
  } catch (er) {
    console.error('summary error', er)
  }
}

const summary = ({ results, counts, lists }) => (
  <Box flexDirection="column">
    ...
    <Box flexDirection="column">
      ...
      <Box flexDirection="column">
        {
          lists.tests
            .filter(t => t.counts.total !== t.counts.pass)
