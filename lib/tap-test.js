...
Test.prototype.timeout = function () {
  // detect false alarms
  if (this._ended || this._bailedOut) return
  this.fail("Timeout!")
  this.end()
}
...
Test.prototype.emit = (function (em) { return function (t) {
  ...
  if (t === "bailout") {
    console.error("Test bailout event", this.listeners(t).length)
    if (this.listeners(t).length === 0 && this.harness) {
      console.error("  bailout bubbling to harness")
      console.error("this.results=", this.results)
      for (var i = 0; i < this.results.length; i ++) {
        this.harness.results.add(this.results[i])
        this.harness.output.write(this.results[i])
      }
      console.error("this.harness.results=", this.harness.results)
