...
Runner.prototype.runFiles = function (files, dir, cb) {
  var self = this
  chain(files.sort(alphasort).map(function(f) {
    return function (cb) {
      ...
      // since we have to read files, go ahead and read it, and
      // just handle EISDIR if as a directory, rather than doing
      // a stat first.
      fs.readFile(f, "utf8", function (er, data) {
        if (er) {
          ...
          self.write(assert.fail("failed to read file " + f, {error: er}))
          return cb()
        }
        ...
        if (shebang) {
          cmd = shebang[1]
          args = (shebang[2] || "").split(" ").concat([fileName])
        ...
        if (doCoverage && path.extname(ilename) === ".js") {
          var foriginal = data
            , fcontents = self.coverHeader() + foriginal + self.coverFooter()
            , tmpBaseName = path.basename(fileName, path.extname(fileName))
                          + ".with-coverage." + process.pid + path.extname(f)
            , tmpFname = path.resolve(path.dirname(fileName), tmpBaseName)
            , i
...
Runner.prototype.coverFooter = function() {
  ...
  , "      fs.writeFile(testFn, JSON.stringify(outObj), function (er) {"
  , "        t.ifError(er, 'failed writing coverage information')"
  , "        t.end()"
  , "      })"
  , "    })"
  , "  })"
