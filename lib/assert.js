  ...
  t.addAssert('type', 2, function (obj, klass, m, e) {
    ...
    if (type === 'object' && klass !== 'object') {
      if (typeof klass === 'function') {
        ...
        e.wanted = name
        return this.ok(obj instanceof type, m, e)
      }
      ...
      do {
        ...
        if (p === klass.prototype || p.constructor === klass) {
          if (p === klass || (p.constructor && p.constructor.name === name))
            return this.pass(m, e)
        }
      } while (p = Object.getPrototypeOf(p))
    }

    this.equal(type, name, m, e)
  })
