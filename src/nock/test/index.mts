...
t.test('patch adds nock feature', async t => {
  ...
  t.ok(
    ...
    'disableNetConnect is present'
  )
  t.ok(
    t.nock.enableNetConnect,
    'enableNetConnect is present'
  )
  t.ok(t.nock.snapshot, 'snapshot is present')
})

t.test(
  'disableNetConnect/enableNetConnect work',
  async t => {
    // create a server
...
t.test('cleans scopes in teardown', async t => {
  ...
  await t.resolves(async () => {
    ...
    t.equal(
      res.status,
      200,
      'but parent nock is still active'
    )
    const body = await res.json()
    t.same(body, { helloFrom: 'parent' })
  ...
  await t.test(
    'child test with an overlapping scope',
    async t => {
      t.nock('http://a.b')
...
t.test(
  'fails tests when mocks are unsatisfied',
  async t => {
    ...
    const fakeTestP = new Promise<void>(
      (resolve, reject) => {
